    ...
    <main className="page-shell">
      ...
      <section className="two-column dashboard-row">
        <article className="panel panel-pad current-seat-card">
          ...
          {seat ? <><div className="split-row"><h2 className="panel-title">{seat.code}</h2><StatusBadge status={seat.status} /></div><p>{seat.label}</p><p className="muted">Your session ends at {seat.occupiedUntil ? new Date(seat.occupiedUntil).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "the end of the grace period"}.</p><Link className="button button-secondary" href="/my-seat">Manage my seat</Link></> : <><h2 className="panel-title">No seat reserved</h2><p className="muted">Browse the live map, then scan the printed QR on an available seat.</p><div className="inline-actions"><Link className="button" href="/seats">Browse seats</Link><Link className="button button-secondary" href="/scan">Scan Seat QR</Link></div></>}
        </article>
